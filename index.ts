...
export default function searchOnYourBrowser(pi: ExtensionAPI) {
  ...
  pi.registerTool({
    ...
    promptGuidelines: [
      ...
      "visit_page accepts an optional `query`. When you only need specific information from a page (not the whole content), pass a focused `query` and only the subagent's concise answer is returned to your context instead of the full page markdown — this keeps the conversation compact for large pages. The subagent reuses your current Pi model by default (no setup needed); pin a different one with /browse. Prefer a query when a page is likely large and you need just a fact or two.",
      "visit_page accepts a `clean` flag. For articles, docs, or blog posts, pass `clean: true` to extract only the main article content as clean Markdown (drops nav/sidebars/ads/links) — far fewer tokens. No effect on X/Reddit/Amazon/Scholar. For large articles, combine `clean: true` with a `query` so the subagent reads only the clean article and returns a concise answer.",
    ],
    parameters: Type.Object({
      ...
      clean: Type.Optional(
        Type.Boolean({
          description:
            "Extract only the page's main article content as clean Markdown (via Defuddle) instead of the default block-walker. Drops navigation, sidebars, ads, footers, and the visible-links dump — far fewer tokens. Best for articles, docs, blog posts. No effect on X/Reddit/Amazon/Scholar (already clean). Falls back to the generic extractor if Defuddle fails.",
        }),
      ),
      query: Type.Optional(
        Type.String({
          description:
            "Optional question about the page. When provided, the full page content is read by a configurable subagent model (see /browse) and only its concise answer is returned — the raw page markdown is NOT added to your chat context. Use this when you need specific information from a large page and want to keep the conversation compact. If the subagent is not configured, the call returns an error with setup instructions.",
        }),
      ),
    }),
